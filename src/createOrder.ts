...
import { AnyFn, OrderWaitOpts, EnsurePromise } from './types'

type Listener<F extends AnyFn> = (p: EnsurePromise<F>, args: Parameters<F>) => void
...
export default function createOrder <Orderer extends AnyFn> (orderer: Orderer) {
    ...
    const order = (...args: Parameters<Orderer>) => {
        const promise = Promise.resolve(orderer(...args)) as EnsurePromise<Orderer>
    ...
    order.useWait = ({ delay= 0, duration= 0, filter= returnTrue }: OrderWaitOpts<Orderer> = {}) => {
        ...
        const filterer = useCallback(filter[0], [1])
